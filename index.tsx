 ...
 */
import { GoogleGenAI, Chat, Content, GenerateContentResponse, GenerateContentParameters, SendMessageParameters } from "@google/genai";
...
interface ChatMessage {
  ...
  messageType?: 'text' | 'image'; // New field
  imageData?: { // New field for image messages
      base64: string;
      mimeType: string;
      promptForImage: string; // The prompt used for this specific image
  };
}
...
let recognition: typeof SpeechRecognition extends new () => infer R ? R : any;
...
function showScreen(screenId: string) {
  ...
  if (screenId === CHAT_SCREEN_ID) {
    ...
    (document.querySelector(`input[name="ai_tone"][value="${currentAiTone}"]`) as HTMLInputElement).checked = true;
    if (darkModeToggle) darkModeToggle.checked = darkModeEnabled;
    if (ttsToggle) ttsToggle.checked = ttsEnabled;
    if (internetSearchToggle) internetSearchToggle.checked = internetSearchEnabled;
    if (deepThinkingToggle) deepThinkingToggle.checked = deepThinkingEnabled;
...
function getSystemInstruction(tone: string, profile: UserProfile, isDeepThinking: boolean, isInternetSearch: boolean): string {
    let baseInstruction = "";
  ...
  const history: Content[] = session.messages
    ...
    .map(msg => {
        if (msg.messageType === 'image' && msg.imageData) {
             // For history, we might just pass the prompt that led to the image,
             // or a placeholder indicating an image was exchanged.
             // The current Gemini chat history expects text parts.
            return {
                role: msg.sender === "User" ? "user" : "model",
                parts: [{ text: msg.imageData.promptForImage || "[User sent an image instruction]" }] // Or msg.text if it's the user's prompt
            }
        }
        return {
            role: msg.sender === "User" ? "user" : "model",
...
function appendMessage(
  senderName: string,
  textOrData: string, // For text type, this is 'text'. For image type, this is the original prompt for the image.
  type: 'user' | 'ai',
...
// Fix: thinkingBudget should be 0 to disable thinking
    ...
    if (internetSearchEnabled) {
        modelRunConfig.tools = [{ googleSearch: {} }];
    }
    ...
    if (Object.keys(modelRunConfig).length > 0) {
        sendMessageParams.config = modelRunConfig as GenerateContentParameters["config"];
    }
...
// Fix: Correctly access groundingMetadata and map chunks
    ...
    if (fullResponseText && currentChatSessionId) {
        const session = chatSessions.find(s => s.id === currentChatSessionId);
        if (session) {
            const aiMessageForHistory: ChatMessage = {
                id: aiMessageId, 
                sender: 'Nova',
            ...
            if (aiMessageDiv) { // Final update to the streamed message
                appendMessage("Nova", fullResponseText, 'ai', false, aiMessageDiv, false, groundingSources, aiResponseLang, aiMessageId, 'text');
            } else { // Should not happen if streaming started, but as a fallback
...
// Fix: Ensure numberOfImages is a number
            ...
            if (currentChatSessionId) {
                ...
                if (session) {
                    const aiImageMessageForHistory: ChatMessage = {
                        id: aiImageId,
                        sender: 'Nova',
                        text: `[Image generated for prompt: ${prompt.substring(0,50)}...]`, // Placeholder text for history
                        timestamp: Date.now(),
                        messageType: 'image',
                        imageData: imageDataPayload,
                        detectedLanguage: 'en' // Or detect language of prompt
                    };
        ...
        disableChatInput(false, false); // Reset both loading states
        if (chatInput && !voiceModeActive) chatInput.focus();
    }
}
...
if (recognition) {
    ...
    recognition.onend = () => {
        ...
        micButton?.setAttribute('aria-label', 'Use microphone');
         ...
         if (generateImageChatButtonElement) generateImageChatButtonElement.disabled = isLoading || isImageLoading; // Only re-enable if not otherwise loading
    };
...
function setupEventListeners() {
    ...
    aiToneRadios?.forEach(radio => {
        radio.addEventListener('change', (event) => {
            currentAiTone = (event.target as HTMLInputElement).value;
            saveSetting('aiTone', currentAiTone);
        });
    });
    ...
    let previousScreenForSettings: string | null = CHAT_LIST_SCREEN_ID;
    ...
    document.body.addEventListener('click', (event) => {
      ...
      if (target.closest('.download-in-chat-image-btn')) { // For dynamically added buttons
          const button = target.closest('.download-in-chat-image-btn') as HTMLButtonElement;
...
function addProcessLogEntry(text: string, type: 'info' | 'source' = 'info', url?: string) {
...
// Fix: Use correct model name for image generation
        ...
        if (response.generatedImages && response.generatedImages.length > 0) {
            currentGeneratedImagesData = response.generatedImages.map(imgInfo => ({
                ...
                prompt: prompt, // Use the original prompt for all images in this batch
...
function displayGeneratedImages(imagesData: { base64: string, prompt: string, mimeType: string }[]) {
    ...
    imageStudioGridElement.innerHTML = ''; // Clear previous
    ...
    imagesData.forEach((imgData, index) => {
        ...
        imgElement.onclick = () => openInAppImageViewer(imageSrc); // Open in viewer on click
